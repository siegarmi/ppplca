...
bio_luluc = bd.Database("biosphere luluc regionalized")
bio_pm = bd.Database("biosphere pm regionalized")

#load data to create electricity mixes for China and US based on production loaction dsitribution of pea, soy, and wheat
...
#function to create electricity mixes
...
#load countries for which the model works
...
#function to test if a country lies in Europe
...
#create cultivation processes for pea, soy, and wheat for missing countries based on closest country covered by agrifootprint database
#exchanges are switched to new country wherever possible
...
#create processes for "heat production, natural gas, at boiler modulating >100kW" for all countries based on example of CA-QC by switching exchanges to new country wherever possible
for country in countries["Code"]:
    if country == "CN":
        ...
        if test_activity == []:
                print("Creating heat production activity for " + country + ".")
...
end_time = time.time()
print(f"Everything took {end_time - start_time:.4f} seconds")